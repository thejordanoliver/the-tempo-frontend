...
function WNBAStackedGameCard({ game }: { game: BasketballGame }) {
  ...
  const home = getWNBATeam(homeId);
  const away = getWNBATeam(awayId);

  const homeName = home?.fullName ?? home?.name ?? game?.teams?.home.name;
  const awayName = away?.fullName ?? away?.name ?? game?.teams?.away.name;

  const homeLogo = getWNBATeamLogo(homeId, isDark);
  const awayLogo = getWNBATeamLogo(awayId, isDark);
...
export default memo(WNBAStackedGameCard);
