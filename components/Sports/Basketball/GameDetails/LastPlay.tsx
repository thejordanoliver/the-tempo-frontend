...
export default function LastPlay({
  ...
  const getPlayerAvatar = (espnId?: string, fallbackUrl?: string): string => {
    if (!espnId) return fallbackUrl || placeholder;
    const player =
      homePlayers.find((p) => String(p.id) === String(espnId)) ||
      awayPlayers.find((p) => String(p.id) === String(espnId));
    return player?.headshot_url || fallbackUrl || placeholder;
  };
