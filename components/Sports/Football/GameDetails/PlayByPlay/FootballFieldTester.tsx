...
export type FootballFieldTestPlay =
  | "fieldGoal"
  | "touchdown"
  | "run"
  | "pass";

type FootballFieldAnimationTesterProps = Omit<
  FootballFieldProps,
  "play" | "playId" | "playSequenceNumber" | "drives"
> & {
  initialPlay?: FootballFieldTestPlay;
};
...
function createTestPlay(
  type: FootballFieldTestPlay,
  awayTeamId?: string | number | null,
  awayCode?: string,
  replayKey = 0,
) {
  const team = {
    id: awayTeamId ?? "test-away",
    code: awayCode ?? "AWAY",
    abbreviation: awayCode ?? "AWAY",
  };

  const common = {
    id: `animation-test-${type}-${replayKey}`,
    sequenceNumber: replayKey,
    start: {
      team,
      distance: 10,
    },
  ...
  const plays: Record<FootballFieldTestPlay, PlayObject> = {
    fieldGoal: {
      ...common,
      type: { text: "Field Goal Good", abbreviation: "FG" },
      text: "42 yard field goal is GOOD",
      scoreValue: 3,
      statYardage: 42,
      start: { ...common.start, yardsToEndzone: 25 },
      end: { ...common.end, yardsToEndzone: 25 },
    } as PlayObject,
    touchdown: {
      ...common,
      type: { text: "Passing Touchdown", abbreviation: "PASSING TD" },
      text: "Pass complete for a touchdown",
      scoreValue: 6,
      statYardage: 25,
      yardsAfterCatch: 7,
      start: { ...common.start, yardsToEndzone: 25 },
      end: { ...common.end, yardsToEndzone: 0 },
    } as PlayObject,
    run: {
      ...common,
      type: { text: "Rush", abbreviation: "RUSH" },
      text: "Runner left tackle for 12 yards",
      statYardage: 12,
      start: { ...common.start, yardsToEndzone: 42 },
      end: { ...common.end, yardsToEndzone: 30 },
    } as PlayObject,
    pass: {
      ...common,
      type: { text: "Pass Reception", abbreviation: "PASS" },
      text: "Pass complete for 24 yards",
      statYardage: 24,
      yardsAfterCatch: 6,
      start: { ...common.start, yardsToEndzone: 48 },
      end: { ...common.end, yardsToEndzone: 24 },
    } as PlayObject,
  };
...
function createDriveShape({
  ...
  const isTouchdown =
    getScoreValue(play.scoreValue) === 6 ||
    getPlaySearchText(play).includes("touchdown");
...
export function FootballFieldAnimationTester({
  initialPlay = "fieldGoal",
  awayCode = "AWAY",
  homeCode = "HOME",
  awayTeamId,
  ...fieldProps
}: FootballFieldAnimationTesterProps) {
  const [testPlay, setTestPlay] = useState<FootballFieldTestPlay>(initialPlay);
  const [replayKey, setReplayKey] = useState(0);

  const play = useMemo(
    () => createTestPlay(testPlay, awayTeamId, awayCode, replayKey),
    [awayCode, awayTeamId, replayKey, testPlay],
  );

  const runTest = (nextPlay: FootballFieldTestPlay) => {
    setTestPlay(nextPlay);
  ...
  return (
    <View style={{ gap: 12 }}>
      ...
      >
        {(
          ["fieldGoal", "touchdown", "run", "pass"] as const
        ).map((playType) => {
          const selected = testPlay === playType;
          const label =
            playType === "fieldGoal"
              ? "Field Goal"
          ...
          return (
            <Pressable
              ...
              accessibilityLabel={`Test ${label} animation`}
              onPress={() => runTest(playType)}
              style={({ pressed }) => ({
                minHeight: 40,
                justifyContent: "center",
                borderRadius: 999,
                paddingHorizontal: 14,
                backgroundColor: selected ? "#1877F2" : "#34363A",
        ...
        <Pressable
          accessibilityRole="button"
          accessibilityLabel="Replay current animation"
          onPress={() => setReplayKey((currentKey) => currentKey + 1)}
          style={({ pressed }) => ({
            minHeight: 40,
            justifyContent: "center",
            borderWidth: 1,
        ...
        >
          <Text
            style={{
              color: fieldProps.isDark === false ? "#111111" : "#FFFFFF",
              fontFamily: Fonts.BOLD,
              fontSize: 13,
            }}
          >
