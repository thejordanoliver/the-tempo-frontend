...
type Props = {
  ...
  onSelectEvent: (index: number, event: UFCCalendarEvent) => void;
  isDark: boolean;
...
function getEventLabel(event: UFCCalendarEvent) {
  ...
  const dateLabel = dayjs(event.startDate).format("MMM D");

  if (label.includes(":")) {
    const [eventName] = label.split(":");
...
export default function UFCEventSelector({
  events,
  loading = false,
  ...
  const safeSelectedIndex = useMemo(() => {
    ...
    return Math.min(Math.max(selectedEventIndex, 0), sortedEvents.length - 1);
  }, [sortedEvents.length, selectedEventIndex]);
  ...
  const computeScrollOffset = useCallback(
    (index: number) => {
      if (!containerWidth || !sortedEvents.length) return 0;
      ...
      const totalWidth =
        SIDE_PADDING * 2 +
        sortedEvents.length * itemWidth +
        ITEM_SPACING * Math.max(sortedEvents.length - 1, 0);

      const maxOffset = Math.max(0, totalWidth - containerWidth);

      return Math.max(0, Math.min(targetOffset, maxOffset));
    },
    [containerWidth, sortedEvents.length, itemStep, itemWidth],
  );
  ...
  const handleSelectEvent = useCallback(
    ...
    [computeScrollOffset, onSelectEvent, sortedEvents],
  );
  ...
  }, [safeSelectedIndex, itemStep, sortedEvents.length, indicatorX]);

  useEffect(() => {
    if (!scrollViewRef.current || !containerWidth || !sortedEvents.length) {
      return;
    }
  ...
  if (loading && !sortedEvents.length) {
