...
import { CBBRecruitTeamRanking } from "@/hooks/BasketballHooks/useCBBTeamRecruitingRankings";
...
export default function TeamRankCard({ item, index }: Props) {
  ...
  const team = teamId ? getCBBTeam(Number(teamId)) : null;
  const logoId = team?.id ?? teamId;
  const logo = logoId ? getCBBTeamLogo(logoId, isDark) : null;
