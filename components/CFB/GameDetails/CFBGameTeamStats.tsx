...
export default function CFBGameTeamStats({
  ...
  const getTeamColor = (teamId?: number) => {
    ...
    if (isDark) {
      ...
      if (team.code === "UTEP") return "#fff";
      if (team.code === "MTSU") return primary;
      if (team.code === "TB") return primary;
      if (team.code === "USF") return "#fff";
  ...
  const teamALogo = getTeamLogo(teamA?.team.id, isDark);
  const teamBLogo = getTeamLogo(teamB?.team.id, isDark);
  ...
  return (
    <View key={key} style={styles.statSection}>
      ...
      <View style={styles.row}>
        <Text style={styles.barText}>{valueB}</Text>
        ...
        <Text style={styles.barText}>{valueA}</Text>
      </View>
    </View>
  );
})}
...
const gameTeamStatsStyles = (isDark: boolean, lighter: boolean) =>
  StyleSheet.create({
    ...
    dividerLine: {
      height: 1,
      width: "100%",
      backgroundColor: lighter ? Colors.darkGray : isDark ? Colors.darkGray : Colors.lightGray,
    },
    statLabel: {
