...
import { teams, getTeamLogo } from "constants/teamsCFB";
...
function getOpponentCodeFromName(opponentName: string): string | undefined {
  const team = teams.find(
    (t) => t.name === opponentName || t.code === opponentName
  );
  return team?.code;
}
...
export default function LastFiveGamesSwitcher({
  ...
  const getLogo = (teamCode: string) => {
    // If lighter → always logoLight
    // If not lighter → dark mode = logoLight, light mode = regular logo
    return lighter || isDark ? getTeamLogo(teamCode, true) : getTeamLogo(teamCode, false);
  };
  ...
  const renderRow = ({ item, index }: { item: any; index: number }) => {
    ...
    const opponentLogoSource = isPlaceholder ? null : getLogo(opponentCode || "TBD");
    ...
    return (
      <View
        style={[
          styles.row,
          {
            borderBottomColor: lighter ? "#888" : isDark ? "#444" : "#ccc",
      ...
      <View style={{ alignSelf: "center" }}>
        <FixedWidthTabBar
          ...
          renderLabel={(tab, isSelected) => {
            ...
            const logoSource = getLogo(teamData.teamCode);
