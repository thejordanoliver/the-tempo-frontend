...
const getCacheKey = (params: {
  ...
  `nfl_event_odds:${params.homeId}:${params.awayId}:${params.date}:${
    params.markets ?? "default"
  }:${params.bookmakers ?? "all"}`;

export function useNFLEventOdds({
  homeId,
  awayId,
  date,
  markets,
  ...
  useEffect(() => {
    ...
    const fetchOdds = async () => {
      try {
        ...
        // 2️⃣ Fetch from API
        const res = await axios.get(`${BASE_URL}/api/nfl/odds/events/odds`, {
          params: {
            homeId,
            awayId,
            date: normalizedDate,
        ...
        console.error("❌ Failed to fetch NFL event odds", err);
