...
export const useFootballGamePossession = (
  ...
  // --- choose the correct team mapping based on league ---
  const espnToInternal = useMemo(() => {
    const map: Record<string, number> = {};
    const selectedTeams = league === "cfb" ? cfbTeams : nflTeams;
  ...
  const skipFetch =
    !homeEspnId || !awayEspnId || !date || !homeInternalId || !awayInternalId;
    ...
    async (isPolling = false) => {
      ...
      try {
        ...
        if (axios.isAxiosError(err) && err.response?.status === 404) {
          console.warn("[Possession] Game not found — stopping polling");
          fatalErrorRef.current = true;
          setError("Game not found");
          return;
        }

        console.warn("[Possession] Error:", err);
        setError(err.message || "Failed to fetch possession");
      } finally {
        setLoading(false);
      }
    },
    [homeEspnId, awayEspnId, date, league, skipFetch, espnToInternal]
  );
