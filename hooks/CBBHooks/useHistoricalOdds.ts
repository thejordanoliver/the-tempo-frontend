...
export const useHistoricalOdds = (options: UseHistoricalOddsOptions) => {
  const { date, timestamp, team1, team2, team1Id, team2Id, gameId, skip } =
    options;
  ...
  useEffect(() => {
    ...
    // Prevent duplicate requests with identical params
    ...
    const fetchData = async () => {
      ...
      try {
        const res = await axios.get(
          `${BASE_URL}/api/cbb/odds/historical`,
          {
            params,
        ...
        if (team1Id && team2Id) {
          games = games.filter(
            (g) =>
              (g.home_team_id === team1Id &&
                g.away_team_id === team2Id) ||
              (g.home_team_id === team2Id &&
                g.away_team_id === team1Id)
          );
        } else if (team1 && team2) {
          games = games.filter(
            (g) =>
