...
export interface HistoricalCFBGameOdds {
  id: string;
...
interface UseHistoricalCFBOddsOptions {
...
const cache: Record<string, HistoricalCFBGameOdds[]> = {};

export const useHistoricalCFBOdds = (options: UseHistoricalCFBOddsOptions) => {
  const { date, timestamp, team1, team2, skip } = options;

  const [data, setData] = useState<HistoricalCFBGameOdds[]>([]);
...
if (timestamp) params.timestamp = new Date(timestamp).toISOString();
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        let games: HistoricalCFBGameOdds[] = res.data.games || [];
        ...
        setError(
          err.response?.data?.error || err.message || "Failed to fetch CFB historical odds"
        );
      } finally {
        setLoading(false);
      }
